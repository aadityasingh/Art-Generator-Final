...
class DCDiscriminator(nn.Module):
    def __init__(self, conv_dim=64):
        ...
        self.linear = linear(conv_dim * 8 * 8 * 8, 1, batch_norm = False)

        self.cd = self.conv_dim

    def forward(self, x):

        out = F.relu(self.conv1(x))    # BS x 64 x 64 x 16
        out = F.relu(self.conv2(out))  # BS x 128 x 32 x 8
        out = F.relu(self.conv3(out))  # BS x 256 x 16 x 4
        out = F.relu(self.conv4(out)).view(-1, self.cd * 8 * 8 * 8)
        out = self.linear(out)

...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = ((D_X(images_X) - 1)**2).sum()/len(images_X)
        D_Y_loss = ((D_Y(images_Y) - 1)**2).sum()/len(images_Y)
        ...
        fake_X, logvar_X, mu_X = G_YtoX(images_Y) 

        g_loss = ((D_X(fake_X) - 1)**2).sum()/len(fake_X) #MSE
        g_loss += -0.5 * torch.sum(1 + logvar_X - mu_X.pow(2) - logvar_X.exp()) #KLD

        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            cycle_consistency_loss = ((images_Y - reconstructed_Y)**2).sum()/len(images_Y)
        ...
        if opts.use_cycle_consistency_loss:
            ...
            cycle_consistency_loss = ((images_X - reconstructed_X)**2).mean()
            g_loss += cycle_consistency_loss
...
def main(opts, dataloader_X, test_dataloader_X):
    ...
    # Start training
    training_loop(dataloader_X, dataloader_X, test_dataloader_X, test_dataloader_X, opts)
